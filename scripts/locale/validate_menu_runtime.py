#!/usr/bin/env python3
"""
Menu runtime verification (CI) for hybrid menu architecture:
- Rust `app_menu.rs` keeps native shell handlers only (no Rust top-bar menu build)
- macOS uses `@tauri-apps/api/menu` from the frontend (`macNativeAppMenu.ts`)
- Win/Linux use the in-app React menu bar
...
UI_ZH_TW = ROOT / "src" / "i18n" / "locales" / "zh-TW.json"
...
STRICT_UI_LOCALES: tuple[tuple[str, pathlib.Path], ...] = (
    ("zh-CN", UI_ZH),
    ("zh-TW", UI_ZH_TW),
)


def check_strict_menu_ui_translations(errors: list[str]) -> None:
    """Every menu.* key in en.json must exist in zh-CN / zh-TW sparse UI locales (menu.enforcer)."""
    if not UI_EN.is_file():
        errors.append("missing src/i18n/locales/en.json")
        return

    en_ui = {k: v for k, v in load_json(UI_EN).items() if isinstance(k, str) and isinstance(v, str)}
    menu_keys = sorted(k for k in en_ui if k.startswith("menu."))

    for locale_id, ui_path in STRICT_UI_LOCALES:
        if not ui_path.is_file():
            errors.append(f"missing UI locale file: {ui_path.relative_to(ROOT)}")
            continue
        raw = {k: v for k, v in load_json(ui_path).items() if isinstance(k, str) and isinstance(v, str)}
        for key in menu_keys:
            val = raw.get(key, "")
            if not val.strip():
                errors.append(f"{locale_id} missing menu translation: {key}")
...
def main() -> int:
    ...
    check_strict_menu_ui_translations(errors)
    ...
    print("OK: hybrid menu runtime (macOS native TS menu, Win/Linux React bar, zh-CN Edit P0, guard)")
    return 0
