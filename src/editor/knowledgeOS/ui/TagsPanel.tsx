...
type Props = {
  ...
  onViewInGraph?: (tag: string) => void
}

export function TagsPanel({ focusTag = null, onViewInGraph }: Props) {
  const { t } = useI18n()
      ...
      <div className="kos-tags-scroll">
        ...
        {activeTag ? (
          <section className="kos-backlink-section kos-tags-docs">
            <div className="kos-tags-docs-header">
              <h3 className="kos-section-title">{t('knowledge.tags.notes', { count: activeDocs.length })}</h3>
              {onViewInGraph ? (
                <button
                  type="button"
                  className="kos-graph-text-btn kos-tags-view-graph"
                  data-testid="kos-tags-view-in-graph"
                  onClick={() => onViewInGraph(activeTag)}
                >
                  {t('knowledge.tags.viewInGraph')}
                </button>
              ) : null}
            </div>
            {activeDocs.length === 0 ? (
